...
/* PBR variant that consumes per-vertex colors from a separate vertex buffer:
 * rgb multiplies the material albedo, alpha multiplies the material
 * roughness. Used by the vertex-color mesh batch (meshes whose FlecsMesh3
 * has a colors array, e.g. terrain). */
static const char *kShaderSource =
    ...
    "  var albedo = mat_color.rgb * input.color.rgb;\n"
    "  var alpha = mat_color.a;\n"
    "  if (material.layer_albedo != 0u) {\n"
    "    let base_color = sample_albedo(uv, material.layer_albedo, dx, dy);\n"
    "    if (base_color.a < 0.5) { discard; }\n"
    "    albedo = base_color.rgb * mat_color.rgb * input.color.rgb;\n"
    "    alpha = base_color.a * mat_color.a;\n"
    "  }\n"

    "  var roughness = material.roughness * input.color.a;\n"
    "  var metallic = material.metallic;\n"
    "  if (material.layer_mr != 0u) {\n"
    "    let mr = sample_roughness(uv, material.layer_mr, dx, dy);\n"
    "    roughness = mr.g * material.roughness * input.color.a;\n"
    ...
    "      em_sample = sample_emissive(uv, material.layer_emissive, dx, dy).rgb;\n"
    "    }\n"
    ...
    "    let normal_sample = sample_normal(uv, material.layer_normal, dx, dy).rgb;\n"
...
ecs_entity_t flecsEngine_shader_pbrVertexColor(
    ecs_world_t *world)
{
    return flecsEngine_shader_ensure(world, "PbrVertexColorShader",
        &(FlecsShader){
